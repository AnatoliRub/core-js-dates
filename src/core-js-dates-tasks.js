...
function getNextFriday(date) {
  ...
  const reqDays = (5 - dayOfWeek + 7) % 7 || 7;

  return new Date(ms + reqDays * 60 * 60 * 1000 * 24);
}
...
function isDateInPeriod(date, period) {
  ...
  const startInMs = Date.parse(period.start);
  const endInMs = Date.parse(period.end);

  return startInMs <= dateInMs && dateInMs <= endInMs;
}
...
function formatDate(date) {
  ...
  const partOfTheDay = h >= 12 ? 'PM' : 'AM';

  return `${month}/${day}/${year}, ${h % 12 || 12}:${m}:${s} ${partOfTheDay}`;
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  return Array.from({ length: daysInMonth }).reduce((acc, _, i) => {
    const date = new Date(year, month - 1, i + 1);
    const day = date.getDay();

    const weekends = day === 0 || day === 6 ? acc + 1 : acc;
    return weekends;
  }, 0);
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.floor(ms / (60 * 60 * 24 * 7 * 1000)) + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  while (currDate) {
    if (currDate.getDate() === 13 && currDate.getDay() === 5) return currDate;
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const [day, month, year] = period.start.split('-').map((el) => Number(el));
  const [dayEnd, monthEnd, yearEnd] = period.end
    .split('-')
    .map((el) => Number(el));
